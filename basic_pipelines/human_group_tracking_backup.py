"""
Human Group Tracking Pipeline with Custom Model Support
- Detects humans (tiny persons) using custom trained HEF model
...
Usage:
    python human_group_tracking.py --input rpi --hef-path /usr/local/hailo/resources/models/hailo8l/best.hef
"""
...
# Default path for custom trained model
DEFAULT_CUSTOM_HEF = "/usr/local/hailo/resources/models/hailo8l/best.hef"
# Default labels JSON for single-class person model
...
class user_app_callback_class(app_callback_class):
    def __init__(self, clustering_eps=100, clustering_min_samples=2, target_label="person"):
        super().__init__()
        ...
        # Target label for detection (custom model may use different label)
...
def app_callback(pad, info, user_data):
    ...
    # Debug: print all detected labels on first few frames
    if user_data.get_count() <= 5 and len(detections) > 0:
        all_labels = [d.get_label() for d in detections]
        print(f"DEBUG Frame {user_data.get_count()}: All detected labels = {all_labels}")

    # Filter for human detections only
    # For custom single-class model trained on "person", accept class_id=0 or label="person"
    human_detections = []
    human_bboxes = []
    human_centers = []
    ...
    for detection in detections:
        ...
        # Accept if:
        # 1. Label is "person" (standard COCO)
        # 2. Label matches user-specified target
        # 3. Class ID is 0 (for single-class custom models)
        is_person = (
            label.lower() == "person" or 
            label.lower() == user_data.target_label.lower() or
            class_id == 0  # Custom single-class model
        )
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Human Group Tracking with Pan/Tilt Guidance (Custom Model Support)",
    ...
    parser.add_argument(
        ...
        type=str,
        default='person',
        help='Target label to detect (for custom trained models).'
    )
...
if __name__ == "__main__":
    ...
    # Parse command line arguments (our custom args)
    args, remaining_args = parse_arguments()
    
    print("\n" + "="*60)
    print("Human Group Tracking Pipeline (Custom Model)")
    print("="*60)
    ...
    # Rebuild sys.argv with only the arguments that GStreamerDetectionApp understands
    # Keep script name and remaining args (those not consumed by our parser)
    sys.argv = [sys.argv[0]] + remaining_args
    
    # ALWAYS use the custom HEF model - remove any existing --hef-path first
    hef_path = DEFAULT_CUSTOM_HEF
    
    # Remove any existing --hef-path from remaining args
    new_argv = [sys.argv[0]]
    i = 1
    while i < len(sys.argv):
        if sys.argv[i] == '--hef-path':
            # Skip --hef-path and its value
            hef_path = sys.argv[i + 1] if i + 1 < len(sys.argv) else hef_path
            i += 2
        else:
    ...
    # Add our HEF path
    new_argv.extend(['--hef-path', hef_path])
    
    # Add labels JSON for custom single-class model if not already specified
    if '--labels-json' not in new_argv:
        labels_json = DEFAULT_LABELS_JSON
        if Path(labels_json).exists():
            new_argv.extend(['--labels-json', labels_json])
            print(f"Using labels JSON: {labels_json}")
    
    sys.argv = new_argv
    
    print(f"Using HEF model: {hef_path}")
    print(f"Final sys.argv: {sys.argv}")
    
    # Verify the HEF file exists
